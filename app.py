...
# Using the python module tmdb3 to get the info from TheMovieDb.org
# for more info check https://pypi.org/project/tmdb3/
...
# Using this function to get info from movie DB
...
def showItemInfo(catalog_name, item_name):
    ...
    # Check if info found
    # then, get trailer, overview and rating
    if movie:
        # Using the urlparse to parse url and get query from it
        # Pass the query to youtube embed to show the trailer
        m = urlparse(movie[0]).query[2:]
        overview = movie[1]
        rating = movie[2]

    if 'username' not in login_session:
        return render_template('itemInfo.html', items=items, rating=rating,
                               overview=overview, m=m, catalogs=catalogs)
    else:
        return render_template('itemInfologgedin.html',
                               items=items, rating=rating,
                               overview=overview, m=m, catalogs=catalogs)
